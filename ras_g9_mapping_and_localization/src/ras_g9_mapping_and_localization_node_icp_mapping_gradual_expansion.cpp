...
#include <ras_g9_msgs/ObjectIdx.h>
#include <ras_g9_msgs/ObjectIdxArray.h>
#include <vector>
#include <math.h>
#include <iostream>
#include <fstream>
...
#define POSITION_UPDATE_RATE 10.0 //[Hz]
#define OBSTACLE_INFLATION_RADIUS_MAX 0.25 //[m]
#define OBSTACLE_INFLATION_RADIUS_MIN 0.1 //[m]
...
private:
    ...
    ras_g9_msgs::ObjectIdxArray object_array;
...
public:
    ...
    void object_array_callback(const ras_g9_msgs::ObjectIdxArray::ConstPtr& msg) {
        object_array = (*msg);
    }
    ...
    ras_g9_msgs::ObjectIdxArray& get_object_array() {
        return object_array;
    }
...
void mark_map(nav_msgs::OccupancyGrid& map_msg, double pos_x, double pos_y, bool is_wall) {
	...
	*/	
    ...
    if (row >= 0 && row < map_msg.info.height && col >= 0 && col < map_msg.info.width) {
        ...
        if (is_wall) {
            ...
            inflation_value_start = 90;
        }
        else {
            if (map_msg.data[mark_pos_array] < 90) {
                map_msg.data[mark_pos_array]++;
                inflation_value_start =  map_msg.data[mark_pos_array];
            } else {
                inflation_value_start = 90;
            }
        }
    ...
    // Inflate obstacle
    int inflation_radius_max = (int) (OBSTACLE_INFLATION_RADIUS_MAX / map_msg.info.resolution);
    int inflation_radius_min = (int) (OBSTACLE_INFLATION_RADIUS_MIN / map_msg.info.resolution);

    for (int curr_row = -inflation_radius_max; curr_row <= inflation_radius_max; curr_row++) {
        for(int curr_col = -inflation_radius_max; curr_col <= inflation_radius_max; curr_col++) {
            int distance_squared = curr_row * curr_row + curr_col * curr_col;
            if (distance_squared <= inflation_radius_max * inflation_radius_max) {
                if ((row+curr_row) >= 0 && (row+curr_row) < map_msg.info.height && (col+curr_col) >= 0 && (col+curr_col) < map_msg.info.width) {
                    double distance = std::sqrt(distance_squared);               

                    int inflation_value = inflation_value_start;
                    if (distance > inflation_radius_min) { // Gradual descent
                        inflation_value = round(inflation_value_start - inflation_value_start * ((distance - inflation_radius_min) / ((double) (inflation_radius_max - inflation_radius_min)) ) );
                    }

                    /*
                    if (!is_wall && inflation_value > 90) {
                        std::cout << "err: dist, inf start, inf" << distance << " " << inflation_value_start << " " << inflation_value << std::endl;
                    }
                    */

                    int mark_pos_array = (row+curr_row) * map_msg.info.width + (col+curr_col);
                    if (map_msg.data[mark_pos_array] < inflation_value) {map_msg.data[mark_pos_array] = inflation_value;}
                }
            }
        }
    }
...
void draw_wall_map(nav_msgs::OccupancyGrid& map_msg, double wall_start_x, double wall_start_y, double wall_end_x, double wall_end_y) {
	...
	*/
	...
	while (true) {
        mark_map(map_msg, current_x, current_y, true);
...
void translate_laser_scan_xy(std::vector<point_xy_t>& points_xy, const sensor_msgs::LaserScan& laser_scan) {
	...
	for (int i=0;i<nr_of_points;i++) {
		...
		if (laser_scan.ranges[i] < laser_scan.range_max && laser_scan.ranges[i] > laser_scan.range_min) {
            ...
            if (!(std::abs(point.y) < 0.085 && point.x < 0.1 && point.x > -0.17)) {points_xy.push_back(point);}
...
void forget_obstacles_in_map(nav_msgs::OccupancyGrid& map_msg) {
    for (int row = 0; row < map_msg.info.height; row++) {
        for (int col = 0; col < map_msg.info.width; col++) {
            int mark_pos_array = row * map_msg.info.width + col;
            if (map_msg.data[mark_pos_array] > 0 && map_msg.data[mark_pos_array] <= 90) {map_msg.data[mark_pos_array]--;}
        }
    }

    // Re-add wall inflations
    for (int row = 0; row < map_msg.info.height; row++) {
        for (int col = 0; col < map_msg.info.width; col++) {
            int mark_pos_array = row * map_msg.info.width + col;
            if (map_msg.data[mark_pos_array] == 100) {
                double x = col * map_msg.info.resolution + map_msg.info.resolution / 5.0;
                double y = row * map_msg.info.resolution + map_msg.info.resolution / 5.0;

                //std::cout << "x y" << x << " " << y << std::endl;
                mark_map(map_msg,x,y,true);
            }
        }
    }
...
void add_obstacles_in_map(nav_msgs::OccupancyGrid& map_msg, const std::vector<point_xy_t>& points_xy, ras_g9_msgs::ObjectIdxArray& object_array) {
    std::vector<point_xy_t>::const_iterator it;
    for (it = points_xy.begin(); it != points_xy.end(); ++it) {
        mark_map(map_msg,it->x,it->y,false);
	}

    std::vector<ras_g9_msgs::ObjectIdx>::const_iterator itt;
    for (itt = object_array.objects.begin(); itt != object_array.objects.end(); ++itt) {
        ras_g9_msgs::ObjectIdx current_object = *itt;

        if (current_object.confidence < 0.5) {continue;}

        double object_radius = 0;
        if (current_object.object.data < 14) {
            object_radius = 0.022;
        } else if (current_object.object.data == 14) {
            object_radius = 0.05;
        } else if (current_object.object.data == 15 || current_object.object.data == 16) {
            object_radius = 0.07;
        }

        mark_map(map_msg,current_object.x,current_object.y,false);
    }
...
int main(int argc, char **argv) {		
    ...
    ros::Subscriber object_aray_sub = n.subscribe("/object_map",1,&Callback_class::object_array_callback,&callback_object);
    ...
    // Read map from file:
    std::vector<map_file_wall> map_file_walls;
    ...
    if (map_file.is_open()) {
        while ( getline (map_file,line) ) {
            ...
            map_file_wall map_wall;
            std::stringstream line_stream(line);
            line_stream >> map_wall.wall_start_x >> map_wall.wall_start_y >> map_wall.wall_end_x >> map_wall.wall_end_y;

            double max_x_wall = std::max(map_wall.wall_start_x,map_wall.wall_end_x);
            double max_y_wall = std::max(map_wall.wall_start_y,map_wall.wall_end_y);

            if (max_x_wall > max_x) {max_x = max_x_wall;}
            if (max_y_wall > max_y) {max_y = max_y_wall;}

            map_file_walls.push_back(map_wall);
        }
        map_file.close();
    ...
    map_msg.info.width = (int) (max_x / map_msg.info.resolution) + 1; // Map width [cells]
    map_msg.info.height = (int) (max_y / map_msg.info.resolution) + 1; // Map height [cells]
    ...
    for (std::vector<map_file_wall>::iterator it = map_file_walls.begin(); it != map_file_walls.end(); ++it) {
        map_file_wall map_wall = *it;

        draw_wall_map(map_msg,map_wall.wall_start_x,map_wall.wall_start_y,map_wall.wall_end_x,map_wall.wall_end_y);

        point_xy_t point_a, point_b;
        point_a.x = map_wall.wall_start_x;
        point_a.y = map_wall.wall_start_y;
        point_b.x = map_wall.wall_end_x;
        point_b.y = map_wall.wall_end_y;
    ...
    ros::Rate r(POSITION_UPDATE_RATE);
	while(n.ok()) {
        ...
        double delta_rot_robot = orientation_z_robot;
        ...
        // Mapping:
        delta_rot_robot = (orientation_z_robot - delta_rot_robot);
        if (std::abs(delta_rot_robot) < 0.01 && !callback_object.is_arm_in_use()) {
            if (ite%10 == 0) {forget_obstacles_in_map(map_msg);}
            if (ite%8 == 0) {add_obstacles_in_map(map_msg,laser_scan_xy,callback_object.get_object_array());}
        }
