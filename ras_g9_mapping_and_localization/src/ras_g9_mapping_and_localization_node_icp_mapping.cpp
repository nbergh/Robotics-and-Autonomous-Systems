...
#define MAP_UPDATE_RATE 1 //[Hz]
#define OBSTACLE_INFLATION_RADIUS 0.18 //[m]
...
void mark_map(nav_msgs::OccupancyGrid& map_msg, double pos_x, double pos_y, double inflation_radius_param, bool is_forgettable_obstacle) {
	...
	*/	
	...
	if (row >= 0 && row < map_msg.info.height && col >= 0 && col < map_msg.info.width) {
        ...
        if (!is_forgettable_obstacle) {
            map_msg.data[mark_pos_array] = 100;
        }
	}
	
	// Inflate obstacle
    int inflation_radius = (int) (inflation_radius_param / map_msg.info.resolution);
		...
		for(int curr_col = -inflation_radius; curr_col <= inflation_radius; curr_col++) {
            if ((curr_row * curr_row + curr_col * curr_col) <= inflation_radius * inflation_radius) {
				...
				if ((row+curr_row) >= 0 && (row+curr_row) < map_msg.info.height && (col+curr_col) >= 0 && (col+curr_col) < map_msg.info.width) {
                    ...
                    if (!is_forgettable_obstacle) {
                        if (map_msg.data[mark_pos_array] < 100) {map_msg.data[mark_pos_array] = 90;}
                    } else {
                        if (map_msg.data[mark_pos_array] < 80) {map_msg.data[mark_pos_array]++;}
                    }
				}					
			}		
		}
...
void draw_wall_map(nav_msgs::OccupancyGrid& map_msg, double wall_start_x, double wall_start_y, double wall_end_x, double wall_end_y) {
	...
	*/
	...
	while (true) {
        mark_map(map_msg, current_x, current_y, OBSTACLE_INFLATION_RADIUS, false);
...
void forget_obstacles_in_map(nav_msgs::OccupancyGrid& map_msg) {
    for (int row = 0; row < map_msg.info.height; row++) {
        for (int col = 0; col < map_msg.info.width; col++) {
            int mark_pos_array = row * map_msg.info.width + col;
            if (map_msg.data[mark_pos_array] > 0 && map_msg.data[mark_pos_array] <= 80) {map_msg.data[mark_pos_array]--;}
        }
    }
}
...
void add_obstacles_in_map(nav_msgs::OccupancyGrid& map_msg, const std::vector<point_xy_t>& points_xy, ras_g9_msgs::ObjectIdxArray& object_array) {
    ...
    for (it = points_xy.begin(); it != points_xy.end(); ++it) {
        mark_map(map_msg,it->x,it->y,OBSTACLE_INFLATION_RADIUS*0.8,true);
	}
    ...
    for (itt = object_array.objects.begin(); itt != object_array.objects.end(); ++itt) {
        ...
        mark_map(map_msg,current_object.x,current_object.y,object_radius + 0.8*OBSTACLE_INFLATION_RADIUS, true);
    }
...
int main(int argc, char **argv) {		
	...
	while(n.ok()) {
        ...
        if (std::abs(delta_rot_robot) < 0.01) {
            if (ite%10 == 0) {forget_obstacles_in_map(map_msg);}
            if (ite%2 == 0) {add_obstacles_in_map(map_msg,laser_scan_xy,callback_object.get_object_array());}
        }
